# copiado de https://raw.githubusercontent.com/geopy/geopy/master/geopy/geocoders/arcgis.py
...
class ArcGISSuggest(ArcGIS):
    """Geocoder using the ERSI ArcGIS API for Suggestions
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.api = (
            '%s://geocode.arcgis.com/arcgis/rest/services/'
            'World/GeocodeServer/suggest' % self.scheme
        )
    ...
    def geocode(self, query, exactly_one=True, timeout=DEFAULT_SENTINEL,
        ...
        # Success; convert from the ArcGIS JSON format.
        geocoded = []
        for resource in response['suggestions']:
            if not resource["isCollection"]:
                geocoded.append(
                    {
                        "text": resource["text"],
                        "magickey": resource["magicKey"]
                    }
                )
        if exactly_one:
            return geocoded[0]
